# ============================================================
# IMAGINATION v1.2.1 — Sleek minimal UI, model lifecycle, cloaking
#
...
CHAT_MODE_CHOICES: List[Tuple[str, str]] = [
    ("Imagination 1.2.1 (auto subagents)", "chat"),
    ("Search the web", "web"),
    ("Coder", "coder"),
]
# Top bar: only v1.2 ships in this build; v1.3 is a visible placeholder.
MODEL_VERSION_DEFAULT = "i12"
MODEL_VERSION_CHOICES: List[Tuple[str, str]] = [
    ("Imagination 1.2.1", "i12"),
]
...
def chat_submit(
    ...
    if (model_version or MODEL_VERSION_DEFAULT).strip() != "i12":
        err = (
            "Only **Imagination 1.2** is available in this build. "
            "**Imagination 1.3** is coming soon — stay tuned."
        )
        bad_disp = display_state + [{"role": "user", "content": user_msg}, {"role": "assistant", "content": err}]
        yield (
            bad_disp,
            conversation_state,
            display_state,
    ...
    if use_web:
        ...
        if not use_cloaker:
            ...
            yield working_display, conversation_state, display_state, "Reasoning…", HIDDEN_PROGRESS, acc_line
    ...
    if task_id == TaskId.CHAT_MAIN:
        ...
        yield working_display, conversation_state, display_state, "Image generation not yet implemented in v1.2.1", HIDDEN_PROGRESS, acc_line
        return
    else:
        tok, mdl, lock = RUNTIME.main_tokenizer, RUNTIME.main_model, GEN_LOCKS["main"]
    ...
    try:
        ...
        try:
            task_label_log = next((s.label for s in specs if s.id == task_id), "Imagination 1.2.1")
        ...
        task_label_short = next((s.label for s in specs if s.id == task_id), "Imagination 1.2.1")
...
def build_ui(auth_http_available: bool = False):
    ...
    with gr.Blocks(css=CSS, theme=theme, title="Imagination v1.2.1") as demo:
        ...
        with gr.Column(elem_id="shell"):
            ...
            with gr.Row(elem_id="model-bar"):
                ...
                gr.HTML(
                    ...
                    '<span class="mode-coming-soon" title="Coming soon">Imagination 1.3</span>'
                    '<span class="mode-soon-sep">·</span>'
                    '<span class="mode-coming-soon" title="Coming soon">Reasoning</span>'
                    '<span class="mode-soon-sep">·</span>'
                    '<span class="mode-coming-soon" title="Coming soon">Research</span>'
                    '<span class="mode-soon-sep">·</span>'
                    '<span class="mode-coming-soon" title="Coming soon">Image</span>'
                )
                ...
                gr.HTML('<span class="term-version">v1.2.1</span>')
            ...
            with gr.Row(elem_classes=["terminal-split"]):
                with gr.Column(elem_classes=["terminal-col"]):
                    ...
                    with gr.Column(elem_classes=["terminal-panel"]):
                        user = gr.Textbox(
                            placeholder="Message…",
                            show_label=False,
                            container=False,
...
<form method="post" action="/auth/email/signup" style="display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;margin:10px 0;">
  ...
  <button type="submit" style="padding:10px 18px;border-radius:12px;background:transparent;border:1px solid rgba(255,255,255,0.2);color:#e8ecff;font-weight:600;cursor:pointer">Create account</button>
            ...
            def chat_inputs():
                return [model_version, chat_mode, user, conversation_state, display_state, user_session]

#!/usr/bin/env python3
"""
Imagination v1.2.1 — Google Colab one-shot setup.
...
def main() -> str:
    ...
    """
    print("Imagination v1.2.1 — Colab setup\n")
    ...
    v121 = Path(content_root) / "versions" / "v1.2.1"
    if not v121.is_dir():
        print(f"[setup] Warning: expected app folder missing: {v121}")
...
def _launch_gradio(content_root: str) -> None:
    ...
    v121 = Path(content_root) / "versions" / "v1.2.1"
    if not v121.is_dir():
        raise FileNotFoundError(f"Missing app folder: {v121}")
    os.chdir(str(v121))
    p = str(v121)
    ...
    from imagination_v1_2_1 import build_ui, preload_main_model
